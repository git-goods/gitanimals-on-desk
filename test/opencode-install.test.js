...
function makeTempConfigDir(initial) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "gitanimals-opencode-install-"));
...
describe("opencode plugin installer", () => {
  it("creates opencode.json when missing and registers the plugin path", () => {
    const configPath = path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "gitanimals-opencode-install-")),
      "opencode.json",
    );
    tempDirs.push(path.dirname(configPath));
    const pluginDir = "/fake/gitanimals/hooks/opencode-plugin";
  ...
  it("does not stomp scoped npm packages named opencode-plugin", () => {
    ...
    // naive basename check would clobber the scoped package. GitAnimals only ever
  ...
  it("updates stale Windows absolute plugin paths", () => {
    ...
    const staleWin = "C:/old/gitanimals/hooks/opencode-plugin";
    const configPath = makeTempConfigDir({ plugin: [staleWin] });
    const pluginDir = "/new/gitanimals/hooks/opencode-plugin";
  ...
  it("skips silently when ~/.config/opencode/ does not exist (no configPath override)", () => {
    ...
    const fakeHome = path.join(os.tmpdir(), `gitanimals-opencode-no-config-${Date.now()}`);
...
describe("resolvePluginDir", () => {
  ...
  it("returns a path ending with /opencode-plugin and uses forward slashes", () => {
    const result = resolvePluginDir("/app/gitanimals/hooks");
    assert.ok(result.endsWith("/opencode-plugin"), `got: ${result}`);
    assert.ok(!result.includes("\\"), `backslashes leaked: ${result}`);
    assert.ok(result.includes("/app/gitanimals/hooks/"), `base dir missing: ${result}`);
  });

  it("replaces app.asar with app.asar.unpacked for packaged builds", () => {
    const result = resolvePluginDir("/Applications/GitAnimals.app/Contents/Resources/app.asar/hooks");
    assert.ok(
  ...
  it("leaves non-asar paths unchanged apart from suffix append", () => {
    const result = resolvePluginDir("/home/user/gitanimals-dev/hooks");
    assert.ok(result.endsWith("/home/user/gitanimals-dev/hooks/opencode-plugin"), `got: ${result}`);
