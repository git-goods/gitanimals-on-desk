...
function createPidResolver(options) {
    ...
    return function resolve() {
        ...
        for (let i = 0; i < maxDepth; i++) {
            ...
            try {
                if (isWin) {
                    const out = execFileSync("wmic", ["process", "where", `ProcessId=${pid}`, "get", "Name,ParentProcessId", "/format:csv"], { encoding: "utf8", timeout: 1500, windowsHide: true });
            ...
            pidChain.push(pid);
            if (!detectedEditor && editorMap[name])
                detectedEditor = editorMap[name];
            ...
            if (!agentPid) {
                if (agentNameSet && agentNameSet.has(name)) {
                    ...
                    try {
                        const cmdOut = isWin
                            ? execFileSync("wmic", ["process", "where", `ProcessId=${pid}`, "get", "CommandLine", "/format:csv"], { encoding: "utf8", timeout: 500, windowsHide: true })
