...
function ToastStack({ toasts }) {
    return (React.createElement("div", { className: "toast-stack", id: "toastStack" }, toasts.map((toast) => (React.createElement("div", { key: toast.id, className: cx("toast", toast.error && "error", "visible") }, toast.message)))));
}
function Sidebar({ activeTab, setActiveTab, t }) {
    return (React.createElement("nav", { className: "sidebar", id: "sidebar" }, SIDEBAR_TABS.map((tab) => (React.createElement("div", { key: tab.id, className: cx("sidebar-item", !tab.available && "disabled", tab.id === activeTab && "active"), onClick: tab.available ? () => setActiveTab(tab.id) : undefined },
        React.createElement("span", { className: "sidebar-item-icon" }, tab.icon),
        React.createElement("span", { className: "sidebar-item-label" }, t(tab.labelKey)),
        tab.available ? null : (React.createElement("span", { className: "sidebar-item-soon" }, t("sidebarSoon"))))))));
}
function Section({ title, children }) {
    return (React.createElement("section", { className: "section" },
        title ? React.createElement("h2", { className: "section-title" }, title) : null,
        React.createElement("div", { className: "section-rows" }, children)));
}
function SwitchControl({ on, pending, disabled, onToggle }) {
    ...
    return (React.createElement("div", { className: "row-control" },
        React.createElement("div", { className: cx("switch", on && "on", pending && "pending", disabled && "disabled"), role: "switch", tabIndex: disabled ? -1 : 0, "aria-checked": on ? "true" : "false", onClick: disabled || pending ? undefined : onToggle, onKeyDown: handleKeyDown })));
}
function SettingRow({ label, desc, control, extraClass }) {
    return (React.createElement("div", { className: cx("row", extraClass) },
        React.createElement("div", { className: "row-text" },
            React.createElement("span", { className: "row-label" }, label),
            desc ? React.createElement("span", { className: "row-desc" }, desc) : null),
        control));
}
function UserCard({ t, userInfo, pending, onLogout, onSignInAgain }) {
    return h(Section, { title: "" }, h("div", { className: "row" }, h("div", { className: "row-text" }, h("span", { className: "row-label" }, userInfo
        ? `\u{1F464} @${userInfo.username}`
        : `\u{1F464} ${t("userCardLoading")}`), h("span", { className: "row-desc" }, t("userCardSignedIn"))), h("div", { className: "row-control", style: { display: "flex", gap: "6px" } }, h("button", {
        className: "btn",
        type: "button",
        disabled: pending,
        onClick: onLogout,
    }, t("userCardSignOut")), h("button", {
        className: "btn",
        type: "button",
        disabled: pending,
...
function UpdateSection({ t, updateState, pending, runCommand }) {
    ...
    actionButtons.push(h("button", {
        ...
        onClick: () => runCommand("checkForUpdates", () => window.settingsAPI.command("checkForUpdatesFromSettings")),
    }, t("updateCheckNow")));
    if (updateState && updateState.canApplyUpdate) {
        actionButtons.push(h("button", {
            ...
            onClick: () => runCommand("applyUpdate", () => window.settingsAPI.command("applyUpdateFromSettings")),
    ...
    if (updateState && updateState.canRestartToUpdate) {
        actionButtons.push(h("button", {
            ...
            onClick: () => runCommand("restartToUpdate", () => window.settingsAPI.command("restartToUpdateFromSettings")),
        }, t("updateRestartNow")));
    }
    return h(Section, { title: t("sectionUpdates") }, h("div", { className: "update-card" }, h("div", { className: "update-summary" }, h("div", { className: cx("update-status", status) }, statusText), h("div", { className: "update-meta-grid" }, h("span", { className: "update-meta-label" }, t("updateCurrentVersion")), h("span", { className: "update-meta-value mono" }, `v${currentVersion}`), h("span", { className: "update-meta-label" }, t("updateLatestVersion")), h("span", { className: "update-meta-value mono" }, latestVersion ? `v${String(latestVersion).replace(/^v/, "")}` : "—"), h("span", { className: "update-meta-label" }, t("updateLastChecked")), h("span", { className: "update-meta-value" }, lastCheckedAt
        ? formatDateTime(lastCheckedAt)
        : t("updateNeverChecked"))), updateState
        ? h("div", { className: "update-flow" }, updateState.flow === "git"
            ? t("updateFlowGit")
            : t("updateFlowAuto"))
        : null, lastError
        ? h("div", { className: "update-error-text" }, lastError)
        : null), h("div", { className: "update-actions" }, actionButtons)));
}
function GeneralTab({ snapshot, t, pending, runUpdate, runCommand, userInfo, updateState, }) {
    const soundEnabled = !snapshot.soundMuted;
    return h(React.Fragment, null, h("h1", null, t("settingsTitle")), h("p", { className: "subtitle" }, t("settingsSubtitle")), h(UserCard, {
        t,
        userInfo,
    ...
    }), h(Section, { title: t("sectionAppearance") }, h(LanguageRow, {
        snapshot,
        t,
    ...
    })), h(Section, { title: t("sectionStartup") }, h(ToggleRow, {
    ...
    })), h(UpdateSection, { t, updateState, pending, runCommand }), snapshot.platform === "darwin" &&
        h(Section, { title: t("sectionMacOS") }, h(ToggleRow, {
        ...
        })), h(Section, { title: t("sectionBubbles") }, h(ToggleRow, {
    ...
    })), h(Section, { title: t("sectionPrivacy") }, h(ToggleRow, {
...
function LanguageRow({ snapshot, t, pending, onChange }) {
    ...
    return h("div", { className: "row" }, h("div", { className: "row-text" }, h("span", { className: "row-label" }, t("rowLanguage")), h("span", { className: "row-desc" }, t("rowLanguageDesc"))), h("div", { className: "row-control" }, h("div", { className: "segmented", role: "tablist" }, options.map((option) => h("button", {
        key: option.value,
...
function AgentsTab({ snapshot, t, agentMetadata, pending, runCommand }) {
    return h(React.Fragment, null, h("h1", null, t("agentsTitle")), h("p", { className: "subtitle" }, t("agentsSubtitle")), !agentMetadata || agentMetadata.length === 0
        ? h("div", { className: "placeholder" }, h("div", { className: "placeholder-desc" }, t("agentsEmpty")))
        : h(Section, { title: "" }, agentMetadata.flatMap((agent) => {
            ...
            const rows = [
                h(ToggleRow, {
                    ...
                    onToggle: () => runCommand(`agent:${agent.id}:enabled`, () => window.settingsAPI.command("setAgentFlag", {
                        agentId: agent.id,
                        flag: "enabled",
                        value: !enabled,
                    })),
            ...
            if (caps.permissionApproval || caps.interactiveBubble) {
                rows.push(h(ToggleRow, {
                    ...
                    onToggle: () => runCommand(`agent:${agent.id}:permissionsEnabled`, () => window.settingsAPI.command("setAgentFlag", {
                        agentId: agent.id,
...
function AgentBadges({ agent, t }) {
    const eventSourceKey = agent.eventSource === "log-poll"
        ...
        : agent.eventSource === "plugin-event"
            ? "eventSourcePlugin"
            : "eventSourceHook";
    return h("span", { className: "row-desc agent-badges" }, h("span", { className: "agent-badge" }, t(eventSourceKey)), agent.capabilities && agent.capabilities.permissionApproval
...
function ThemeTab({ snapshot, t, themeMetadata, themeRefreshing, pending, runCommand, refreshThemes, }) {
    ...
    return h(React.Fragment, null, h("div", { className: "tab-header" }, h("h1", null, t("themeTabTitle")), h("button", {
        className: "btn",
        type: "button",
        disabled: themeRefreshing,
        onClick: refreshThemes,
    }, themeRefreshing ? t("themeRefreshing") : t("themeRefresh"))), h("p", { className: "subtitle" }, t("themeTabSubtitle")), themeMetadata == null
        ? h("p", { className: "subtitle" }, "…")
        : h(Section, { title: "" }, themeMetadata.map((theme) => {
            ...
            return h(ToggleRow, {
                ...
                onToggle: () => runCommand(pendingKey, async () => {
        ...
        }, h("span", null, "\uD83D\uDD12"), h("span", null, t("personaInfo")), h("a", {
            href: "#",
            style: {
                color: "#7eb8ff",
...
function PlaceholderTab({ t }) {
    return h("div", { className: "placeholder" }, h("div", { className: "placeholder-icon" }, "\u{1F6E0}"), h("div", { className: "placeholder-title" }, t("placeholderTitle")), h("div", { className: "placeholder-desc" }, t("placeholderDesc")));
}
function AboutTab({ snapshot, t }) {
    ...
    const linkButton = (url) => h("div", { className: "row-control" }, h("button", {
        className: "btn",
        type: "button",
        onClick: openLink(url),
    }, t("aboutOpenLink")));
    return h(React.Fragment, null, h("h1", null, t("sidebarAbout")), h("p", { className: "subtitle" }, t("aboutSubtitle")), h(Section, { title: "" }, h(SettingRow, {
        label: t("aboutVersion"),
        control: h("div", { className: "row-control" }, h("span", { className: "mono" }, `v${version}`)),
    }), h(SettingRow, {
        label: t("aboutLicense"),
        control: h("div", { className: "row-control" }, h("span", null, t("aboutLicenseValue"))),
    }), h(SettingRow, {
...
function App() {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: "app" },
            React.createElement(Sidebar, { activeTab: activeTab, setActiveTab: setActiveTab, t: t }),
            React.createElement("main", { className: "content", id: "content" }, content)),
