 ...
 */
function personaTypeToThemeId(personaType) {
    if (typeof personaType !== "string" || !personaType)
        return null;
    if (Object.keys(_personaTypeMap).length === 0)
        discoverThemes();
    ...
    try {
        for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
            ...
            if (entry.name.startsWith("."))
                continue; // skip dot-dirs like cache metadata
            ...
            try {
                ...
                const declaredPersonaType = (cfg && typeof cfg.personaType === "string" && cfg.personaType) ||
...
function loadTheme(themeId) {
    ...
    if (!raw) {
        ...
        try {
            report("[theme] not found", "error", { themeId, fallingBackToDefault: themeId !== "little-chick" });
        }
        catch { }
        if (themeId !== "little-chick")
            return loadTheme("little-chick");
    ...
    if (errors.length > 0) {
        ...
        try {
            report("[theme] validation errors", "error", { themeId, errors: errors.slice(0, 10) });
        }
        catch { }
        if (themeId !== "little-chick")
            return loadTheme("little-chick");
...
function resolveHint(hookFilename) {
    if (!activeTheme || !activeTheme.displayHintMap)
        return null;
...
function getRendererAssetsPath() {
    if (!activeTheme) {
        try {
            bc("theme", "legacy-fallback", { reason: "no-active-theme" });
        }
        catch { }
        return "../../assets/svg";
    }
...
function mergeDefaults(raw, themeId, isBuiltin) {
    ...
    if (raw.layout && raw.layout.contentBox) {
        ...
        if (theme.layout.centerX == null)
            theme.layout.centerX = cb.x + cb.width / 2;
        if (theme.layout.baselineY == null)
            theme.layout.baselineY = cb.y + cb.height;
    }
    else {
        theme.layout = null;
    }
    ...
    if (Array.isArray(theme.idleAnimations)) {
        for (const a of theme.idleAnimations) {
            if (a && a.file)
                a.file = bn(a.file);
        }
    }
    if (Array.isArray(theme.wideHitboxFiles))
        theme.wideHitboxFiles = theme.wideHitboxFiles.map(bn);
    if (Array.isArray(theme.sleepingHitboxFiles))
        theme.sleepingHitboxFiles = theme.sleepingHitboxFiles.map(bn);
...
function getSoundUrl(soundName) {
    if (!activeTheme || !activeTheme.sounds)
        return null;
