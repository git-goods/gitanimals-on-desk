...
function init(userDataDir) {
    if (!userDataDir)
        throw new Error("remote-theme-sync.init: userDataDir required");
...
function onSyncComplete(fn) {
    if (typeof fn === "function")
        _syncCompleteListeners.push(fn);
}
...
function _notifySyncComplete() {
    for (const fn of _syncCompleteListeners) {
        try {
            ...
            console.warn("[remote-theme-sync] listener error:", e.message);
        }
    }
}
/**
...
function _loadRegistryMeta() {
    if (!themeCacheDir)
        return { fetchedAt: 0, themes: [] };
    const p = path.join(themeCacheDir, ".registry.json");
    try {
        return JSON.parse(fs.readFileSync(p, "utf8")) || { fetchedAt: 0, themes: [] };
    }
    catch {
        return { fetchedAt: 0, themes: [] };
    }
...
async function _syncTheme(themeId, baseUrl, { force = false } = {}) {
    ...
    if (!force && !_isStale(meta.fetchedAt))
        return false; // fresh
    // 1. Fetch theme.json
    ...
    if (!themeRaw || typeof themeRaw !== "object")
        throw new Error("theme.json not an object");
    if (!themeRaw.states || !themeRaw.states.idle)
        throw new Error("theme.json missing states.idle");
...
function _extractThemeFileList(theme) {
    ...
    if (theme.miniMode)
        addFromStates(theme.miniMode.states);
    if (theme.reactions) {
