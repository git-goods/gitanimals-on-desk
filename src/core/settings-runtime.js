...
exports.createSettingsRuntime = createSettingsRuntime;
function createSettingsRuntime({ app, isLinux, loginItemHelpers, settingsController, getLaunchScriptPath = () => "", getWin, getCurrentSize, mini, }) {
    if (!app)
        throw new TypeError("createSettingsRuntime: app is required");
    if (!loginItemHelpers) {
    ...
    if (!mini)
        throw new TypeError("createSettingsRuntime: mini is required");
    function writeSystemOpenAtLogin(enabled) {
        ...
        app.setLoginItemSettings(loginItemHelpers.getLoginItemSettings({
    ...
    function readSystemOpenAtLogin() {
        if (isLinux)
            return loginItemHelpers.linuxGetOpenAtLogin();
        return app.getLoginItemSettings(app.isPackaged ? {} : { path: process.execPath, args: [app.getAppPath()] }).openAtLogin;
    }
    function hydrateSystemBackedSettings() {
        if (settingsController.get("openAtLoginHydrated"))
            return;
        ...
        try {
            ...
            console.warn("GitAnimals: failed to read system openAtLogin during hydration:", err?.message);
        }
