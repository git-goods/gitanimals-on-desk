...
Clawd 데스크톱 펫 — Electron 기반 데스크톱 펫으로, hook 시스템과 로그 폴링으로 AI coding agent 작업 상태를 실시간 감지하여 SVG 애니메이션을 재생한다. 8개 에이전트 지원: **Codex**(command + HTTP hook), **Codex CLI**(JSONL 로그 폴링), **Copilot CLI**(command hook), **Cursor Agent**(stdin/stdout hook), **Gemini CLI**(session JSON 폴링 + command hook), **CodeBuddy**(command + HTTP hook), **Kiro CLI**(command hook), **opencode**(in-process plugin + 역방향 HTTP bridge). Windows, macOS, Linux 지원.
...
node hooks/install.js          # Codex hooks → ~/.Codex/settings.json
...
```
공통 Hook 흐름 (Codex / Copilot / Cursor / Gemini / CodeBuddy / Kiro):
...
권한 결정 (Codex / CodeBuddy / opencode, 블로킹):
...
| 에이전트     | 소스                     | 능력                                     |
| ------------ | ------------------------ | ---------------------------------------- |
| Codex  | command + HTTP hook      | 권한 버블, subagent, 터미널 포커스       |
...
디렉토리별 전체 파일 목록: `src/AGENTS.md`, `hooks/AGENTS.md`, `agents/AGENTS.md`
...
| `src/AGENTS.md`                   | Electron 서브시스템 상세             |
| `hooks/AGENTS.md`                 | Hook 인프라 & opencode 플러그인 상세 |
| `agents/AGENTS.md`                | 에이전트 설정 모듈 & 모니터링 상세   |
